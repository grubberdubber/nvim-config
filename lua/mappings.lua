...
-- Mejor navegación con líneas wrapeadas
...
-- Pegar sin perder el registro (como VS Code paste sobre selección)
...
-- Navegar entre splits (estándar Vim)
...
-- Navegar buffers (estándar NvChad + Tab como IDE)
...
map("n", "gi",         vim.lsp.buf.implementation,  { desc = "LSP: Implementación" })
...
-- ╔══════════════════════════════════════════════════════════════╗
-- ║  🐛 DAP — Debugger (estándar de IDE: F5/F9/F10/F11/F12)     ║
-- ║                                                              ║
...
-- Breakpoint condicional (como IntelliJ click derecho → Add Conditional)
map("n", "<leader>B", function()
  require("dap").set_breakpoint(vim.fn.input "Condición: ")
end, { desc = "DAP: Breakpoint condicional" })

-- Log point (imprime valor sin detener ejecución)
...
-- También mantener <leader>b como alias de F9 (hábito vim)
